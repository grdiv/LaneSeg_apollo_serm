...
import pandas as pd
from dataloaders.utils_zzm import encode_segmap_gray_apollo, encode_segmap_gray_serm
...
class SeRMSegmentation(data.Dataset):
    NUM_CLASSES = 8

    def __init__(self, args,  csv_file, mode='train'):#没加transform
        super(SeRMSegmentation, self).__init__()
        self.args = args
        self.data = pd.read_csv(os.path.join(os.getcwd(), "data_list", csv_file), header=None,names=["image","label"])
        self.mode = mode
        self.images = self.data["image"].values[1:]
    ...
    def __getitem__(self, idx):
        ...
        ori_mask = np.array(Image.open(self.labels[idx]))
        # train_img, train_mask = tr.crop_resize_data(ori_image, ori_mask)
        train_img, train_mask = ori_image, ori_mask
        # Encode
        train_mask = encode_segmap_gray_serm(train_mask)
