...
def get_serm_color_labels():
    return np.asarray([[[0, 0, 0]], #0 bg
                       [[1, 1, 1]], #1 slow down
                       [[2, 2, 2], [3, 3, 3], [4, 4, 4], [5, 5, 5], [6, 6, 6]],  #2 arrow
                       [[7, 7, 7]], # 3 crosswalk
                       [[8, 8, 8], [9, 9, 9],[10, 10, 10], [11, 11, 11], [12, 12, 12]], #4 lines
                       [[13, 13, 13]], #5 stop line
                       [[14, 14, 14]], #6 number
                       [[15, 15, 15]], #7 text
...
def decode_segmap(label_mask, dataset, plot=False):#类别标签转颜色图标记
    if dataset == 'serm':
        n_classes = 8
        label_colours = get_serm_color_labels()
    else:
...
def decode_segmap_gray_apollo(label_mask, plot = False):
    n_classes = 8
    label_gray = get_serm_gray_labels()
